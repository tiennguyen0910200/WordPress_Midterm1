...
if ( ! class_exists( 'Mega_Magazine_About' ) ) {
	...
	class Mega_Magazine_About {
		...
		/**
		 * Singleton instance of Mega_Magazine_About.
		 *
		 * @var Mega_Magazine_About $instance Mega_Magazine_About instance.
		 */
		private static $instance;

		/**
		 * Main Mega_Magazine_About instance.
		 *
		 * @since 1.0.0
		 *
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Mega_Magazine_About ) ) {
				self::$instance = new Mega_Magazine_About;
		...
		public function setup_config() {
			...
			$this->logo_url      = isset( $this->config['logo_url'] ) ? $this->config['logo_url'] : get_template_directory_uri() . '/inc/about-theme/images/site-logo.png';
			$this->logo_link     = isset( $this->config['logo_link'] ) ? $this->config['logo_link'] : esc_url(__('https://www.prodesigns.com/', 'mega-magazine'));
			$this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.', 'mega-magazine' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '" class="button button-primary" style="text-decoration: none;">' . sprintf( esc_html__( 'Get started with %s', 'mega-magazine' ), $this->theme_name ) . '</a></p>' );
		}
		...
		public function load_assets( $hook ) {
			...
			if ( 'appearance_page_' . $this->page_slug === $hook ) {
				...
				wp_enqueue_style( 'mega-magazine-about', get_template_directory_uri() . '/inc/about-theme/css/about.css', array(), '1.0.0' );
				wp_enqueue_script( 'mega-magazine-about', get_template_directory_uri() . '/inc/about-theme/js/about.js', array( 'jquery' ), '1.0.0' );
				$js_vars = array(
					'ajaxurl' => esc_url( admin_url( 'admin-ajax.php' ) ),
				);
				wp_localize_script( 'mega-magazine-about', 'MegaMagazineAboutObject', $js_vars );
			}
		}
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							switch ( $active['needs'] ) {
								...
								case 'install':
									...
									$label = esc_html__( 'Install', 'mega-magazine' );
									break;
								case 'activate':
									$class = 'activate-now button button-primary';
									$label = esc_html__( 'Activate', 'mega-magazine' );
									break;
								case 'deactivate':
									$class = 'deactivate-now button';
									$label = esc_html__( 'Deactivate', 'mega-magazine' );
									break;
							}
							?>
		...
		public function upgrade_to_pro() {
			...
			if ( isset( $upgrade_to_pro['button_link'] ) && ! empty( $upgrade_to_pro['button_link'] ) ) {
				$button_text = esc_html__( 'Upgrade to Pro', 'mega-magazine' );
		...
		public function free_pro() {
		    ...
		    if ( ! empty( $free_pro ) ) {
		        ...
		        if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
		            ...
		            echo '<tr>';
		            echo '<th></th>';
		            echo '<th>' . esc_html__( 'Mega Magazine','mega-magazine' ) . '</th>';
		            echo '<th>' . esc_html__( 'Mega Magazine Pro','mega-magazine' ) . '</th>';
		            echo '</tr>';
		            echo '</thead>';
		            echo '<tbody>';
		            ...
		            echo '<tr class="pt-about-theme-text-center">';
		            echo '<td></td>';
		            $link = 'https://www.prodesigns.com/wordpress-themes/downloads/mega-magazine-pro/';
		            echo '<td colspan="2"><a href="'.$link.'" target="_blank" class="button button-primary button-hero">Mega Magazine Pro</a></td>';
		            echo '</tr>';
		...
		public function get_plugins( $plugin_folder = '' ) {
			if ( ! function_exists( 'get_plugins' ) ) {
				require_once ABSPATH . 'wp-admin/inc/plugin.php';
			}
